...
{
  let {sandbox_signkey} = await
  Payment.init(config).getSignkey();
  return new Payment({
    ...config,
...
async
_request(params, type, cert = false)
{
  // 安全签名
  ...
  urllib.request(this.urls[type], pkg);
...
>
  ...
  >
    ...
    try {
      ...
      req.weixin = await
      this._parse(req.body, 'middleware_' + type);
    } catch (err) {
...
>
  ...
  >
    ...
    try {
      ...
      ctx.request.weixin = await
      this._parse(ctx.request.body, 'middleware_' + type);
    } catch (err) {
...
// 下载对帐单
async
downloadBill(params, format = false)
{
  let pkg = {
    ...params,
    appid: this.appid,
  ...
  let xml = await
  this._request(pkg, 'downloadbill');
  return this._parseBill(xml, format);
}

// 下载资金帐单
async
downloadFundflow(params, format = false)
{
  let pkg = {
    ...params,
    appid: this.appid,
  ...
  let xml = await
  this._request(pkg, 'downloadfundflow', true);
  return this._parseBill(xml, format);
}
